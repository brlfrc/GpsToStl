...
################################################
    ...
    def _combine_text_with_cylindrical_podium(self,text_mesh, podium):
        ...
        indices = range(-len(submeshes) // 2, len(submeshes) // 2)

        for i, submesh in zip(indices, submeshes):
...
################################################
    def magnets_support(self, magnets_radius = 8, magnet_height = 3):
        ...
        if self.podium.shape == "cylindrical":
            ...
            magnet_support.apply_transform(trimesh.transformations.rotation_matrix(np.pi/2, [0, 0, 1]))
            magnet_support2= magnet_support.copy()

            magnet_support.apply_translation([0,self.podium.radius/2, 0])
            magnet_support2.apply_translation([0,-self.podium.radius/2, 0])
...
################################################
    def photo_holder(self, phot_holder_path ='tmp_STL/photo_holder.stl'):
        photo_holder = trimesh.load(phot_holder_path)
        if self.podium.shape == "rectangular":
            photo_holder.apply_transform(trimesh.transformations.rotation_matrix(-np.pi/2, [0, 0, 1]))

            photo_holder.apply_translation([-self.podium.depth/2,-self.podium.width/2 ,self.podium.height/2])
            self.podium_mesh = self.podium_mesh + photo_holder

        if self.podium.shape == "cylindrical":
            photo_holder.apply_transform(trimesh.transformations.rotation_matrix(-np.pi/2, [0, 0, 1]))

            photo_holder.apply_translation([-self.podium.radius, -30 ,self.podium.height/2])
            self.podium_mesh = self.podium_mesh + photo_holder