...
class UphillSTL:
    def __init__(self, path_gps='example/selvino.gpx', selection=True, output_path='tmp_STL/selected_data.txt', fn = 'tmp_STL/uphill_tmp_STL.stl', 
                 scale=100, thickness_multiplier=1,  image_resolution=800, color = [250,0,0,250], smoothing = False, repair= True, min_thickness_percent = 0):
        """
    ...
    def _create_mesh(self):
        ...
        if self.smoothing:
            elevation_matrix = gaussian_filter(self.elevation_matrix, 1)  # smoothing
        else:
            elevation_matrix = self.elevation_matrix

        mesh = numpy2stl(elevation_matrix, fn=self.fn, scale=self.scale, mask_val=0.05, solid=True, min_thickness_percent= self.min_thickness_percent)
        mesh.visual.face_colors= self.color
        return mesh

    def _repair_mesh(self):
        path = os.path.join(r"C:\Users\utente\OneDrive - Universita degli Studi di Milano-Bicocca\Desktop\GpxToStl", self.fn)
